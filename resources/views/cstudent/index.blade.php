    ...
    <div class="panel">
        ...
        <div class="flex items-center justify-between mb-5">
            ...
            <div class="flex items-center space-x-3">
                <a href="{{ route('cstudent.export', request()->query()) }}" class="btn btn-outline-success">
                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path>
                    </svg>
                    Export CSV
                </a>
                <a href="{{ route('cstudent.create') }}" class="btn btn-primary">
        ...
        <div class="mb-5 p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
            <form method="GET" action="{{ route('cstudent.index') }}" class="flex items-center space-x-4">
                <div class="flex-1">
                    <input type="text" 
                           name="search" 
                           value="{{ request('search') }}"
                           placeholder="Search students by name, mobile, email, or subjects..."
                ...
                @if(request('search'))
                    <a href="{{ route('cstudent.index') }}" class="btn btn-outline-secondary">Clear</a>
                @endif
            </form>
        </div>
        ...
        <div class="mb-4" id="bulkActions" style="display: none;">
            <form method="POST" action="{{ route('cstudent.bulk-delete') }}" onsubmit="return confirm('Are you sure you want to delete selected students?')">
                @csrf
        ...
        <tbody>
                    @forelse ($cstudents as $cstudent)
                        @php
                            ...
                            $paymentProgress = $totalFees > 0 ? ($totalPaid / $totalFees) * 100 : 0;
                        @endphp
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-800">
                            <td>
                                <input type="checkbox" name="student_ids[]" value="{{ $cstudent->id }}" class="form-checkbox student-checkbox">
                            </td>
                            <td>
                                <div class="flex items-center">
                                    @if($cstudent->photo)
                                        <img class="w-8 h-8 rounded-full ltr:mr-2 rtl:ml-2 object-cover" 
                                             src="{{ Storage::url($cstudent->photo) }}" alt="Photo">
                                    @else
                                    ...
                                    @endif
                                    <div class="font-semibold">{{ $cstudent->name }}</div>
                                </div>
                            </td>
                            <td>
                                <span class="text-sm">{{ $cstudent->mobile ?? 'N/A' }}</span>
                            </td>
                            <td>
                                <span class="badge badge-outline-primary text-xs">{{ ucfirst($cstudent->competition_level) }}</span>
                            </td>
                            <td>
                    ...
                    <td class="text-center">
                                <div class="flex justify-center space-x-1">
                                    <!-- View Details -->
                                    <a href="{{ route('cstudent.show', $cstudent->id) }}" x-tooltip="View Details">
                                    ...
                                    <!-- Edit -->
                                    <a href="{{ route('cstudent.edit', $cstudent->id) }}" x-tooltip="Edit">
                                    ...
                                    <!-- Single Pay Fees Button for all subjects -->
                                    @if($remaining > 0)
                                        <button type="button" x-tooltip="Pay Fees"
                                            onclick="openFeesModal({{ $cstudent->id }}, {{ $totalFees }}, {{ $totalPaid }})">
                                    ...
                                    <!-- Delete -->
                                    <form action="{{ route('cstudent.destroy', $cstudent->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this student?');" class="inline">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="text-center text-gray-500 py-8">
                                ...
                                <div class="text-lg font-medium mb-2">No competition students found</div>
                                <div class="text-sm text-gray-400 mb-4">Try adjusting your search criteria or add a new student.</div>
                                <a href="{{ route('cstudent.create') }}" class="btn btn-primary">Add First Student</a>
                    </td>
                </tr>
                    @endforelse
        </tbody>
...
<script>
        ...
        function openFeesModal(cstudentId, totalFees, paidAmount) {
